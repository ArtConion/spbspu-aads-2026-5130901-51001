...
namespace vishnyakov
{
  ...
  {
    std::pair< int, int > coords = interpolate(0, 0, 100, 100, 0.5);
  ...
  BOOST_AUTO_TEST_CASE(TraverseSegmentShortDay)
  {
    ...
    SegmentResult result = traverseSegment("A", 0, 0, "B", 100, 0, startTime, true);
  ...
  BOOST_AUTO_TEST_CASE(TraverseSegmentWithNight)
  {
    ...
    SegmentResult result = traverseSegment("A", 0, 0, "B", 500, 0, startTime, true);
  ...
  BOOST_AUTO_TEST_CASE(BuildGreedyRouteSimple)
  {
    ...
    RouteResult result = buildGreedyRoute(points, 0, 0, 5.0);
    ...
    int stopCount = 0;
    for (LCIter< RouteStop > it = result.allStops.cbegin(); it != result.allStops.cend(); ++it)
    {
      ++stopCount;
    }
    BOOST_TEST(stopCount == 3);

    LCIter< RouteStop > it = result.allStops.cbegin();
